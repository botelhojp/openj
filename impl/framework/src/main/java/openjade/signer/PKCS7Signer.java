...
public class PKCS7Signer {
	...
	private CMSSignedDataGenerator setUpProvider() throws Exception {
		...
		ContentSigner signer = new JcaContentSignerBuilder(SIGNATUREALGO).setProvider("BC").build((PrivateKey) (keystore.getKey(alias, password.toCharArray())));
	...
	public boolean verify(byte[] signedBytes){
		boolean verify = true;
		try {
			CMSSignedDataParser sp = new CMSSignedDataParser(new JcaDigestCalculatorProviderBuilder().setProvider("BC").build(), signedBytes);
